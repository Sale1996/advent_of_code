...
int calculate_min_distance(std::string root_city, std::set<std::string> cities, const std::map<std::string, int>& distances)
{
    if(cities.size() == 2)
    {
    ...
    else
    {
        ...
        {            
            int distance = distances.at(root_city + "," + city) + calculate_min_distance(city, cities, distances);
...
int main () 
{
    ...
    for (std::string city : cities)
    {
        int distance = calculate_min_distance(city, cities, distances);

        if (minimal_distance == -1 || distance < minimal_distance)
            minimal_distance = distance;
    }

    std::cout << "Minimal  distance is s [" << minimal_distance << "]" << std::endl;
